...
def get_api_url(account):
    ...
    return 'https://api.tumblr.com/v2/blog/' + blog_name # + '/likes'
...
def apiparse(base, count, offset=0, before=0):
...
''' % (encoding, self.title, css_rel, body_class)
    ...
    def backup(self, account):
        """makes single files and an index for every like on a public Tumblr blog account"""
        ...
        if options.blosxom:
            ...
            save_folder = join(root_folder, options.outdir or account+"'s likes")
        ...
        if options.incremental:
            ...
            except ValueError:  # min()/max() arg is an empty sequence
                pass
        else:
            log(account, "Getting basic information\r")

        # start by calling the API with just a single post
        soup = apiparse(base + "/posts", 1)
        if not soup:
            self.errors = True
            return
        ...
        resp = soup['response']
        blog = resp['blog']
        self.title = escape(blog['name']+"'s likes")
        ...
        last_post = blog['likes']
        if options.count:
            last_post = min(last_post, options.count + options.skip)

        def _backup(posts):
            last_timestamp = 0
            for p in sorted(posts, key=lambda x: x['liked_timestamp'], reverse=True):
                post = post_class(p)
        ...
        try:
            ...
            while i < last_post:
                ...
                if ts <= 0:
                    soup = apiparse(base + "/likes", j - i, offset = i)
                else:
                    soup = apiparse(base + "/likes", j - i, before = ts)
...
class TumblrPost:
    ...
    def get_post(self):
        ...
        post += u'<header><p><a href=\"http://{0}.tumblr.com/\" class=\"tumblr_blog\">{0}</a>:</p>\n'.format(self.creator)
